...
def sum_(z, mask):
    return jnp.sum(z, axis=0, where=mask, initial=0.0)


def product_(z, mask):
    return jnp.prod(z, axis=0, where=mask, initial=1.0)
...
def mean_(z, mask):
    total = jnp.sum(z, axis=0, where=mask, initial=0.0)
    valid_count = jnp.sum(mask, axis=0)
    return total / jnp.maximum(valid_count, 1)
